...
final class LogViewerModel {
    private(set) var entries: [OpenVaultDiagnosticEntry] = []
    private(set) var isRefreshing = false
    private(set) var errorMessage: String?

    var searchText = ""
    var minimumLevel = OpenVaultDiagnosticLevel.debug
    var isPaused = false

    private var startDate = Date.now.addingTimeInterval(-5 * 60)

    var filteredEntries: [OpenVaultDiagnosticEntry] {
    ...
    func run() async {
        OpenVaultLog.application.notice("Opened the native log viewer")
    ...
    func refresh() async {
        ...
        do {
            entries = try await OpenVaultDiagnostics.entries(since: startDate)
            errorMessage = nil
    ...
    func clearView() {
        ...
        entries = []
        errorMessage = nil
        OpenVaultLog.application.notice("Cleared the native log viewer")
    }

    func text(for selectedIDs: Set<OpenVaultDiagnosticEntry.ID>) -> String {
...
struct LogViewerView: View {
    ...
    @State private var selection: Set<OpenVaultDiagnosticEntry.ID> = []
        ...
        .toolbar {
            ToolbarItemGroup {
                Picker("Minimum Level", selection: $model.minimumLevel) {
                    ForEach(OpenVaultDiagnosticLevel.allCases) { level in
    ...
    private var emptyState: some View {
        if let errorMessage = model.errorMessage {
            ...
            ProgressView("Loading OpenVault logs…")
                .controlSize(.large)
        } else {
            ContentUnavailableView(
                "No Matching Logs",
                systemImage: "text.alignleft",
                description: Text("Use OpenVault or change the current filters.")
            )
        }
    }
    ...
    private func color(for level: OpenVaultDiagnosticLevel) -> Color {
        switch level {
        case .debug:
            .secondary
        case .info:
