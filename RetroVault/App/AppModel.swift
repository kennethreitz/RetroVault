...
final class AppModel {
    ...
    func restore() async {
        ...
        RetroVaultLog.application.debug("Restoring the previous RetroVault session")
        do {
            if let session = try await environment.serverConnection.restoredSession() {
                showLibrary(session)
                RetroVaultLog.connection.notice("Restored a paired RomM session")
            } else {
                destination = .connection
                RetroVaultLog.connection.info("No paired RomM session is stored")
            }
        } catch {
            connectionError = error.localizedDescription
            destination = .connection
            RetroVaultLog.connection.error(
    ...
    func connect(serverURL: String, pairingCode: String) async {
        ...
        RetroVaultLog.connection.notice("Pairing with a RomM server")
        do {
            ...
            showLibrary(session)
            RetroVaultLog.connection.notice("Paired with RomM successfully")
        } catch {
            connectionError = error.localizedDescription
            RetroVaultLog.connection.error(
    ...
    func disconnect() async {
        RetroVaultLog.connection.notice("Disconnecting the RomM server")
        ...
        do {
            ...
            libraryModel = nil
            destination = .connection
            RetroVaultLog.connection.notice("Disconnected the RomM server")
        } catch {
            connectionError = error.localizedDescription
            RetroVaultLog.connection.error(
