...
/// land on screen. Laid out to match `RetroVaultVideoUniforms` in the shader.
...
enum LibretroMetalShader {
    static let source = """
        ...
        struct RetroVaultPixelVertex {
            float4 position [[position]];
            float2 textureCoordinate;
        };

        struct RetroVaultVideoUniforms {
            float sourceWidth;
            float sourceHeight;
            float targetWidth;
            float targetHeight;
        ...
        vertex RetroVaultPixelVertex openVaultPixelVertex(
        ...
        fragment float4 openVaultNearestFragment(
            RetroVaultPixelVertex input [[stage_in]],
        ...
        static float4 openVaultSharpBilinear(
            ...
            constant RetroVaultVideoUniforms &uniforms
        ) {
        ...
        fragment float4 openVaultSharpBilinearFragment(
            RetroVaultPixelVertex input [[stage_in]],
            texture2d<float> frame [[texture(0)]],
            constant RetroVaultVideoUniforms &uniforms [[buffer(0)]]
        ) {
