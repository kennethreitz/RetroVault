...
/// The reviewed catalog of Libretro cores embedded in RetroVault.
struct LibretroCoreManifest: Decodable, Sendable {
    ...
    /// Returns whether RetroVault ships a reviewed core for the named RomM system.
    func supportsSystem(
...
enum LibretroInstallationError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .missingManifest:
            """
            RetroVault could not find its bundled Libretro manifest. Build the cores with \
            Scripts/build-libretro-cores.sh, then rebuild RetroVault.
            """
        case let .unsupportedManifestVersion(version):
            "This build contains Libretro manifest version \(version), which RetroVault does not support."
        ...
        case let .invalidManifest(reason):
            "RetroVault could not read its bundled Libretro manifest: \(reason)"
        }
    }
}
...
struct LibretroInstallation: Sendable {
    ...
    ///
    /// RetroVault uses this entry point for integration tests against the same
