...
@_silgen_name("retrovault_libretro_log_callback_pointer")
...
private enum LibretroRuntimeError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .alreadyRunning:
            "RetroVault currently supports one active Libretro session."
...
///
/// The core renders into an offscreen framebuffer. RetroVault reads completed
...
private final class LibretroEngine: @unchecked Sendable, LibretroCallbackTarget {
    ...
    private let queue = DispatchQueue(
        label: "org.kennethreitz.RetroVault.libretro",
        qos: .userInteractive
    )
    ...
    private func runLoop() {
        ...
        do {
            ...
            case let .failed(message):
                emit(
                    .notice(
                        "RetroVault could not restore the previous session. Starting from the beginning."
                    )
                )
                RetroVaultLog.libretro.error(
    ...
    private func persistSaveMemory(from core: LibretroCore) -> Bool {
        ...
        do {
            ...
            emit(.notice("RetroVault could not save battery-backed memory: \(error.localizedDescription)"))
            return false
        }
    }
