...
///
/// RetroVault extracts only a regular file whose extension is declared by the
...
enum GameArchiveError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .invalidArchive(reason):
            return "RetroVault could not read this ZIP archive: \(reason)"
        ...
        case let .contentTooLarge(name):
            return "The archived game \(name) is larger than RetroVault's 20 GB playback cache limit."
        case let .extractionFailed(name, reason):
            return "RetroVault could not extract \(name) from the ZIP archive: \(reason)"
        }
    }
}
