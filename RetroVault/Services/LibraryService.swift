...
actor RomMLibraryService: LibraryServing {
  ...
  func prepareCartridgeSaveForPlay(
    ...
    guard !candidates.isEmpty else {
      // A save RomM holds but RetroVault skipped is otherwise invisible: the
  ...
  private func cachedGameURL(
    ...
    // Before stable content identities were introduced, RetroVault included
...
extension Notification.Name {
  static let openVaultDownloadedGamesDidChange = Notification.Name(
    "org.kennethreitz.RetroVault.downloaded-games-did-change"
  )
}
...
enum LibraryServiceError: LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .notAuthenticated:
      "RetroVault could not find the RomM client token. Reconnect the server and try again."
    case .gameNotFound:
    ...
    case .incompleteSynchronization:
      "RomM returned an incomplete library page. RetroVault kept the previous offline library."
    case .couldNotExportGame(let reason):
      "RetroVault could not export the ROM to Downloads: \(reason)"
    case .userMetadataWritePermissionRequired:
    ...
    case .couldNotPrepareFirmware(let fileName, let reason):
      "RetroVault could not prepare \(fileName) from RomM: \(reason)"
    case .couldNotCacheGame(let reason):
      "RetroVault downloaded the ROM but could not add it to the local library: \(reason)"
    case .couldNotRemoveDownload(let reason):
      "RetroVault could not remove the local ROM: \(reason)"
    case .saveSyncWritePermissionRequired:
    ...
    case .couldNotSyncSave(let reason):
      "RetroVault preserved the local cartridge save but could not sync it to RomM: \(reason)"
    case .gameExportUnavailable:
