...
final class LibraryModel {
  ...
  func refresh(
    ...
    do {
      ...
      RetroVaultLog.library.notice("Library synchronization was cancelled")
    } catch {
      guard synchronizationID == currentSynchronizationID else {
        return
      }
  ...
  func purgeLocalCacheAndResync() async {
    ...
    refreshErrorMessage = nil
    errorMessage = nil
    RetroVaultLog.library.notice("Purging the local library cache")
    ...
    RetroVaultLog.library.notice("Purged the local library cache")
    await refresh()
  }
  ...
  /// Records a ROM promoted into RetroVault's managed local library.
  ///
  ...
  private func apply(_ incomingSnapshot: LibrarySnapshot) {
    ...
    // RomM only records a play made through its own web player, and RetroVault
