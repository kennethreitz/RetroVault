...
enum LibraryCoordinateSpace {
  static let name = "RetroVault.Library"
}
      ...
      return .failed(
        LibraryAlert(
          ...
          message:
            ...
            ?? "RetroVault couldn’t load the game’s playback information."
        )
      )
    }
      ...
      return .failed(
        LibraryAlert(
          ...
          message:
            ...
            ?? "RetroVault couldn’t prepare this game for playback."
        )
      )
    }
    ...
    Task {
      ...
      if result.completedWithoutErrors {
        libraryAlert = LibraryAlert(
          ...
          message:
            result.successfulItemCount == 1
            ? "Added the game to RetroVault’s local library."
            : "Added the selected games to RetroVault’s local library."
        )
      } else {
        libraryAlert = LibraryAlert(
          title:
    ...
    Task {
      ...
      if result.completedWithoutErrors {
        libraryAlert = LibraryAlert(
          ...
          message:
            result.successfulItemCount == 1
            ? "Removed the game’s local ROM from RetroVault."
            : "Removed the selected local ROMs from RetroVault."
        )
      } else {
        libraryAlert = LibraryAlert(
          title:
...
private struct GameDeletionConfirmationSheet: View {
  ...
  private var deletionExplanation: String {
    if deletesFilesFromServer {
      return
        "The selected database records and ROM files will be deleted from the RomM server. This cannot be undone by RetroVault."
    }
    return
