...
enum RomMAPIError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidResponse:
            "The server returned a response RetroVault could not understand."
        case .unauthorized:
            "RomM rejected the client token."
        case .forbidden:
            "The paired token does not have the permissions RetroVault needs."
        case .notFound:
        ...
        case let .transport(error):
            "RetroVault could not reach RomM: \(error.localizedDescription)"
        case .decoding:
...
extension RomMAPIError {
    ...
    /// put RetroVault into an offline state. Cancellation never appears here: it
