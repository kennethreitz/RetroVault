...
enum RetroVaultCoreTool {
    ...
    Usage:
      RetroVaultCoreTool validate <manifest>
      RetroVaultCoreTool build --manifest <path> --output <directory> \
        --work <directory> [--core <id>]... [--sign <identity>]

    Use --sign - for ad-hoc development signing. Release artifacts must supply
    RetroVault's Developer ID Application identity.
    """
}
